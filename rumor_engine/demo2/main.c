...
int water_announce(void) {
  ...
  for (y = 0; y < playfield_y; y++) {
    for (x = 0; x < playfield_x; x++) {
      if (playfield[y*playfield_x + x] == 'w') {
	for (i = 0; i < CREATURES; i++) {
	  e = creatures[i].x - x;
	  f = creatures[i].y - y;
	  e = sqrt(e*e + f*f);
	  if (e < 2) {
	    r = rumor_create_experience(creatures[i].mem.self);
	    r->subject.id = 0;
	    r->object.id = 0;
	    r->action = ACTION_SAW_WATER;
...
int move_creatures(void) {
  ...
  for (i = 0; i < CREATURES; i++) {
	...
	if (creatures[i].x < playfield_x-1)
	  m = 1;
      }
      else if (d == 2) {
	if (creatures[i].y > 0)
	...
	if (creatures[i].y < playfield_y-1)
	  n = 1;
      }

      creature_move(&creatures[i], n, m);
    ...
    /* look for water */
    else if (creatures[i].state == CSTATE_LOOK_WATER) {
      ...
      while (r != NULL) {
	if (r->action == ACTION_SAW_WATER && (r->status & RUMOR_STATUS_TRUE) != 0)
	  break;
	r = r->next;
      }
      if (r == NULL) {
	...
	for (p = 0; p < playfield_y; p++) {
	  for (q = 0; q < playfield_x; q++) {
	    if (playfield[p*playfield_x + q] == 'w') {
	      m = q - creatures[i].x;
	      n = p - creatures[i].y;
	      if (sqrt(m*m + n*n) < 2) {
		d = 1;
	...
	/* it's there ok! */
	  ...
	  rumor_remove_all(&creatures[i].mem.rumor_list, r);
	  rumor_remove_all(&creatures[i].mem.old_rumors_list, r);
	}
      }
    ...
    /* go check out water */
    else if (creatures[i].state == CSTATE_FIND_WATER) {
	...
	if (abs(n)+abs(m) == 1) {
	  creatures[i].state = CSTATE_LOOK_WATER;
	  d = 1;
	}
	else {
	/* move towards water */
...
int display_rumors(void) {
  ...
  for (i = 0; i < CREATURES; i++) {
	...
	printf("%c salivated ", creatures[r->subject.id].symbol);
      if (r->action == ACTION_SAW_WATER)
	printf("%c saw w in (%d, %d) ", creatures[r->original_teller.id].symbol, (int)r->x, (int)r->y);
...
int rumor_generate(struct creature *c, struct creature *t, int ract) {
  ...
  for (i = 0; i < CREATURES; i++) {
	...
	r->action = ract;
	r->priority = actions[r->action].priority;
	r->original_teller.id = creatures[i].id;
	r->original_teller.race = creatures[i].mem.self->race;
	r->teller.id = creatures[i].id;
